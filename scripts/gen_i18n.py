...
def generate_keys_header(
    ...
    lines: List[str] = [
        ...
        "",
        "// Forward declaration for per-language string blobs",
    ...
    for code, name in zip(languages, language_names):
        ...
        lines.append(f"extern const uint16_t STRINGS_{abbrev}_OFFSETS[];")
    ...
    # getStringView helper
    lines.append("struct LanguageStringsView {")
    ...
    lines.append("inline LanguageStringsView getStringView(Language lang) {")
    lines.append("  switch (lang) {")
    for code, name in zip(languages, language_names):
        abbrev = get_lang_abbreviation(code, name)
        lines.append(f"    case Language::{code}:")
        lines.append(
            f"      return {{i18n_strings::STRINGS_{abbrev}_DATA, i18n_strings::STRINGS_{abbrev}_OFFSETS}};"
        )
    first_abbrev = get_lang_abbreviation(languages[0], language_names[0])
    lines.append("    default:")
    lines.append(
        f"      return {{i18n_strings::STRINGS_{first_abbrev}_DATA, i18n_strings::STRINGS_{first_abbrev}_OFFSETS}};"
    )
    lines.append("  }")
    lines.append("}")
    lines.append("")
...
def generate_strings_cpp(
    ...
    # Per-language string blobs + offsets
    lines.append("namespace i18n_strings {")
    lines.append("")

    for lang_idx, (code, name) in enumerate(zip(languages, language_names)):
        abbrev = get_lang_abbreviation(code, name)
        entries = [translations[key][lang_idx] for key in string_keys]
        offsets: List[int] = []
        byte_offset = 0
        for text in entries:
        ...
        lines.append(f"const uint16_t STRINGS_{abbrev}_OFFSETS[] = {{")
        _append_integer_list(lines, offsets)
        lines.append("};")
        lines.append("")
    ...
    for code, name in zip(languages, language_names):
        abbrev = get_lang_abbreviation(code, name)
        lines.append(
            f"static_assert(sizeof(i18n_strings::STRINGS_{abbrev}_OFFSETS) "
            f"/ sizeof(i18n_strings::STRINGS_{abbrev}_OFFSETS[0]) =="
        )
        lines.append("                  static_cast<size_t>(StrId::_COUNT),")
        lines.append(f'              "STRINGS_{abbrev}_OFFSETS size mismatch");')
...
def _append_blob_string(lines: List[str], text: str, indent: str = "    ") -> None:
    """Append one NUL-terminated string literal chunk for blob data."""
    segments = escape_cpp_string(text)
    if not segments:
        segments = [""]
    segments[-1] += "\\0"
...
def _append_integer_list(lines: List[str], values: List[int], indent: str = "    ", wrap_at: int = 16) -> None:
    """Append a wrapped integer initializer list body."""
    for start in range(0, len(values), wrap_at):
        chunk = values[start : start + wrap_at]
        suffix = "," if start + wrap_at < len(values) else ""
        lines.append(f"{indent}{', '.join(str(v) for v in chunk)}{suffix}")
