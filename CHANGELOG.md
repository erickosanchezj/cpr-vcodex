...
| Version | Changes |
|---|---|
| `Unreleased` | - Synced the fork with selected upstream changes after `e28918b`, including multiple OPDS server support, long branch name display, browser title/header cleanup, long filename footnote navigation fixes, keyboard cursor behavior fixes, Swedish keyboard translations, and Spanish/Italian translation tweaks.<br>- Wired the new keyboard hint translation keys into the vCodex keyboard UI so generated i18n data and runtime text stay aligned.<br>- Kept Bookerly intentionally by not applying upstream's Bookerly replacement, and skipped the royalty.dev add/revert pair because it has no final firmware effect. |
